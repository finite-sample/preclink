...
class MarginFilter:
    ...
    def filter(self, pairs: pd.DataFrame) -> pd.DataFrame:
        ...
        for left_idx in column(pairs, "left_index").unique():
            left_pairs = rows(
                pairs, column(pairs, "left_index") == left_idx
            ).sort_values("score", ascending=False)

            if len(left_pairs) == 1:
                keep_indices.append(left_pairs.index[0])
            elif len(left_pairs) >= 2:
                best_score = column(left_pairs, "score").iloc[0]
                second_score = column(left_pairs, "score").iloc[1]
