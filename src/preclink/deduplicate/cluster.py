...
class ClusterDeduplicator:
    ...
    def deduplicate(self, df: pd.DataFrame) -> tuple[pd.DataFrame, DeduplicationReport]:
        ...
        pairs = rows(pairs, column(pairs, "left_index") < column(pairs, "right_index"))
        ...
        duplicates = rows(scored, column(scored, "score") >= self.threshold)
        ...
        for members in components.values():
            ...
            if size == 1:
                ...
                groups_found += 1
                member_quality = quality.loc[quality.index.isin(members)]
                scores = list(
                    zip(member_quality.index, member_quality.values, strict=True)
                )
