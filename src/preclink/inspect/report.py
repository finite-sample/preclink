...
def generate_report(
    ...
    if not filtered_pairs.empty and "score" in filtered_pairs.columns:
        ambiguous_indices = []
        for left_idx in column(filtered_pairs, "left_index").unique():
            left_pairs = rows(
                filtered_pairs, column(filtered_pairs, "left_index") == left_idx
            )
            if len(left_pairs) >= 2:
                scores = column(left_pairs, "score").sort_values(ascending=False)
