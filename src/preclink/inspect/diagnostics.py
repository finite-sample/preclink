...
from preclink._typing import as_float, column
...
def compute_diagnostics(
    ...
    n_matched_left = column(matches, "left_index").nunique() if not matches.empty else 0
    n_matched_right = (
        column(matches, "right_index").nunique() if not matches.empty else 0
    )

    score_stats: dict[str, float] = {}
    if not matches.empty and "score" in matches.columns:
        scores = column(matches, "score")
        score_stats = {
            "min": as_float(scores.min()),
            "max": as_float(scores.max()),
            "mean": as_float(scores.mean()),
            "median": as_float(scores.median()),
            "std": as_float(scores.std()),
        }
