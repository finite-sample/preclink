    ...
    def block(self, left: pd.DataFrame, right: pd.DataFrame) -> pd.DataFrame:
        ...
        for field in self.on:
            if self.crosswalk is not None:
                left_copy[f"_block_{field}"] = self.crosswalk.apply(
                    column(left_copy, field)
                )
                right_copy[f"_block_{field}"] = self.crosswalk.apply(
                    column(right_copy, field)
                )
            else:
        ...
        pairs = left_blocked.merge(
            right_blocked, on=block_cols, suffixes=("_left", "_right")
        )
...
class FullBlocker:
    ...
    def block(self, left: pd.DataFrame, right: pd.DataFrame) -> pd.DataFrame:
        ...
        return pairs.drop(columns=["_key"])
