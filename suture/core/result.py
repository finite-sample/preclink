...
from suture.inspect.diagnostics import LinkageDiagnostics
from suture.inspect.report import InspectionReport
...
class LinkageResult:
    ...
    def inspect(self, margin_threshold: float = 0.1) -> InspectionReport:
        ...
        """
        from suture.inspect.report import generate_report
    ...
    def merge_left(self, suffixes: tuple[str, str] = ("", "_matched")) -> pd.DataFrame:
        ...
        match_data = self.matches[["left_index"] + right_cols].copy()
    ...
    def merge_right(self, suffixes: tuple[str, str] = ("_matched", "")) -> pd.DataFrame:
        ...
        match_data = self.matches[["right_index"] + left_cols].copy()
        match_data = match_data.rename(
